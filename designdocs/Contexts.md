...
- To simplify xpchainj programming in future.
...
Since the very first version xpchainj has had the concept of "network parameters": a class that wrapped various
...
This proliferation of global variables makes it harder for developers to use multiple instances of xpchainj
simultaneously, for example, to do cross-chain trading of different cryptocurrencies against each other.

Finally, several xpchainj objects currently need to be plugged together in ways that aren't always obvious for full
...
NetworkParameters appears everywhere in the xpchainj API, and so introducing Context will have a major impact on it
