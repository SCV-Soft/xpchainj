...
import io.xpchainj.params.AbstractXpchainNetParams;
...
public class XpchainURI {
    ...
    /**
     * URI for Bitcoin network. Use {@link AbstractXpchainNetParams#BITCOIN_SCHEME} if you specifically
     ...
     * @throws XpchainURIParseException if the URI is not syntactically or semantically valid.
     */
    public XpchainURI(String uri) throws XpchainURIParseException {
        this(null, uri);
    }
     ...
     *
     * @throws XpchainURIParseException If the input fails Bitcoin URI syntax and semantic checks.
     */
    public XpchainURI(@Nullable NetworkParameters params, String input) throws XpchainURIParseException {
        checkNotNull(input);

        String scheme = null == params
            ? AbstractXpchainNetParams.BITCOIN_SCHEME
            : params.getUriScheme();
        ...
        try {
            ...
            throw new XpchainURIParseException("Bad URI syntax", e);
        }
        ...
        if (inputLc.startsWith(blockchainInfoScheme)) {
            ...
            throw new XpchainURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?", 2);
        if (addressSplitTokens.length == 0)
            throw new XpchainURIParseException("No data found after the bitcoin: prefix");
        ...
        if (!addressToken.isEmpty()) {
            ...
            try {
                ...
                throw new XpchainURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new XpchainURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws XpchainURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new XpchainURIParseException("Malformed Bitcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new XpchainURIParseException("Malformed Bitcoin URI - empty name '" +
                        nameValuePairToken + "'");
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    ...
                    if (params != null && amount.isGreaterThan(params.getMaxMoney()))
                        throw new XpchainURIParseException("Max number of coins exceeded");
                    if (amount.signum() < 0)
     ...
     */
    private void putWithValidation(String key, Object value) throws XpchainURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new XpchainURIParseException(String.format(Locale.US, "'%s' is duplicated, URI is invalid", key));
        } else {
     ...
     *         10,000,000 units is 1 XPC. May be null if no amount is specified
     */
