...
package io.xpchainj.walletfx.controls;
...
import io.xpchainj.walletfx.overlay.OverlayController;
import io.xpchainj.walletfx.overlay.OverlayableStackPaneController;
import io.xpchainj.walletfx.utils.GuiUtils;
import io.xpchainj.walletfx.utils.QRCodeImages;
...
/**
 * A custom control that implements a clickable, copyable Xpchain address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableXpchainAddress extends AnchorPane implements OverlayController<ClickableXpchainAddress> {
    ...
    @Override
    public void initOverlay(OverlayableStackPaneController overlayableStackPaneController, OverlayableStackPaneController.OverlayUI<? extends OverlayController<ClickableXpchainAddress>> ui) {
        rootController = overlayableStackPaneController;
    }

    /**
     * @param theAppName The application name to use in Xpchain URIs
     */
    ...
    public ClickableXpchainAddress() {
        try {
    ...
    public String uri() {
        return XpchainURI.convertToBitcoinURI(address.get(), null, appName, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final OverlayableStackPaneController.OverlayUI<ClickableXpchainAddress> overlay = rootController.overlayUI(pane, this);
