 ...
 * xpchainj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params, int eventHorizon, Coin feePerKb, boolean ensureMinRequiredFee) {
        log.info("Creating xpchainj {} context.", VersionMessage.XPCHAINJ_VERSION);
    ...
    /**
     * Returns the current context that is associated with the <b>calling thread</b>. xpchainj is an API that has thread
     ...
     * independently use xpchainj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a xpchainj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using xpchainj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing xpchainj via a thread that has not had any context set on it.");
     ...
     * want to create core XpchainJ objects. Generally, if a class can accept a Context in its constructor and might
