...
package io.xpchainj.wallettool;

import io.xpchainj.core.Sha256Hash;
import io.xpchainj.core.TransactionOutput;
import io.xpchainj.crypto.*;
import io.xpchainj.params.RegTestParams;
import io.xpchainj.protocols.payments.PaymentProtocol;
import io.xpchainj.protocols.payments.PaymentProtocolException;
import io.xpchainj.protocols.payments.PaymentSession;
import io.xpchainj.script.Script;
import io.xpchainj.script.Script.ScriptType;
import io.xpchainj.script.ScriptException;
import io.xpchainj.store.*;
import io.xpchainj.uri.XpchainURI;
import io.xpchainj.uri.XpchainURIParseException;
import io.xpchainj.utils.BriefLogFormatter;
import io.xpchainj.utils.Network;
import io.xpchainj.wallet.CoinSelection;
import io.xpchainj.wallet.CoinSelector;
import io.xpchainj.wallet.DeterministicKeyChain;
import io.xpchainj.wallet.DeterministicSeed;
...
import io.xpchainj.core.DumpedPrivateKey;
import io.xpchainj.core.ECKey;
import io.xpchainj.core.FullPrunedBlockChain;
...
import io.xpchainj.wallet.KeyChainGroupStructure;
import io.xpchainj.wallet.MarriedKeyChain;
...
/**
 * A command line tool for manipulating wallets and working with Xpchain.
 */
...
public class WalletTool implements Callable<Integer> {
    ...
    @CommandLine.Option(names = "--addr", description ="Specifies a Xpchain address, either segwit or legacy.")
    ...
    private static io.xpchain.protocols.payments.Protos.PaymentRequest paymentRequest;
        ...
        if (location.startsWith("http") || location.startsWith("bitcoin")) {
            try {
                ...
                if (location.startsWith("http")) {
                    ...
                    XpchainURI paymentRequestURI = new XpchainURI(location);
                    future = PaymentSession.createFromXpchainUri(paymentRequestURI, verifyPki);
                }
            ...
            try {
                paymentRequest = io.xpchain.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
