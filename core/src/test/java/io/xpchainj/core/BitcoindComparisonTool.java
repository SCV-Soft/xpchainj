...
import io.xpchainj.utils.BlockFileLoader;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/xpchainj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and xpchainj
...
public class BitcoindComparisonTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("USAGE: xpchainjBlockStoreLocation runExpensiveTests(1/0) [port=18444]");
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    rulesSinceFirstFail++;
                    log.error("ERROR: bitcoind and xpchainj acceptance differs on block \"" + block.ruleName + "\"");
                }
                if (block.sendOnce)
